...
export const Component = () => {
  ...
  const onSendChatMessage = useCallback(
    async (content: string, attachments?: UploadedFile[]) => {
      ...
      if (slashCmd) {
        if (legalChatService.isCrossCheckCommand(slashCmd.command)) {
          setIsChatBusy(true);
          try {
            const now = new Date().toISOString();
            legalChatService.appendMessages([
            ...
            const crossCheckSummary = await legalChatService.runCrossCheckFromChat({
              caseId: selectedCaseId,
              workspaceId,
              sessionId: activeChatSessionId,
            });
            ...
            legalChatService.appendMessages([
              {
                ...
                sessionId: activeChatSessionId,
                role: 'assistant',
                content: crossCheckSummary,
                mode: 'general',
                status: 'complete',
                sourceCitations: [],
                normCitations: [],
                findingRefs: [],
                tokenEstimate: Math.ceil(crossCheckSummary.length / 3.5),
                createdAt: new Date().toISOString(),
                updatedAt: new Date().toISOString(),
              },
            ]);
          } catch (error) {
            console.error('[workspace-chat] crosscheck failed', error);
            setTransientStatus('Cross-Check konnte nicht ausgeführt werden.');
          } finally {
            setIsChatBusy(false);
          }
          return;
        ...
        if (legalChatService.isMemoryCommand(slashCmd.command)) {
          const memoryArgs = slashCmd.args.trim();
          if (slashCmd.command === 'gedaechtnis' || (slashCmd.command === 'memory' && !memoryArgs)) {
            setIsChatBusy(true);
            try {
              const now = new Date().toISOString();
              legalChatService.appendMessages([
              ...
              const memoryStatus = await legalChatService.showMemoryStatus({
                workspaceId,
                caseId: selectedCaseId,
              });
              ...
              legalChatService.appendMessages([
                {
                  ...
                  normCitations: [],
                  findingRefs: [],
                  tokenEstimate: Math.ceil(memoryStatus.length / 3.5),
                  createdAt: new Date().toISOString(),
                  updatedAt: new Date().toISOString(),
                },
              ]);
            } catch (error) {
              console.error('[workspace-chat] memory status failed', error);
              setTransientStatus('Gedächtnis konnte nicht geladen werden.');
            } finally {
              setIsChatBusy(false);
            }
            return;
          ...
          if (!memoryArgs) {
            setTransientStatus('Bitte Inhalt angeben, z. B. /merke Mandant bevorzugt formelle Ansprache.');
            return;
          }

          effectiveContent = `Merke dir: ${memoryArgs}`;
  ...
  const onSubmitMessageFeedback = useCallback(
    async (
      messageId: string,
      rating: 'positive' | 'negative' | 'neutral',
      comment?: string
    ) => {
      if (!activeChatSessionId || !selectedCaseId) return;

      const targetMessage = activeChatMessages.find(msg => msg.id === messageId);
      await legalChatService.submitMessageFeedback({
        messageId,
        sessionId: activeChatSessionId,
        caseId: selectedCaseId,
        workspaceId,
        rating,
        comment,
        mode: targetMessage?.mode ?? activeChatMode,
      });
      setTransientStatus(
        rating === 'positive'
          ? 'Danke für das Feedback.'
          : rating === 'negative'
            ? 'Feedback gespeichert. Ich lerne daraus.'
      ...
      <ViewBody>
        <div className={styles.chatRoot}>
          <PremiumChatSection
            ...
            onSubmitMessageFeedback={(messageId, rating, comment) => {
              onSubmitMessageFeedback(messageId, rating, comment).catch(() => {});
            }}
          />
        </div>
      </ViewBody>
