...
const SLASH_COMMANDS = [
  ...
  { command: '/crosscheck', description: 'Cross-Check gegen Akte', example: '/crosscheck' },
  { command: '/merke', description: 'Im Copilot-Gedächtnis speichern', example: '/merke Mandant bevorzugt formelle Ansprache' },
  { command: '/gedaechtnis', description: 'Gedächtnis anzeigen', example: '/gedaechtnis' },
];
...
const ChatBubble = ({ message, isChatBusy, onRegenerate, onDelete, onSaveInsight, onUndoInsight, onSaveArtifactToAkte, onResolveToolApproval, onSubmitMessageFeedback }: {
  ...
  const hasIntelligence = !isUser && (!!message.confidence || !!message.reasoningChain);
  ...
  const submitFeedback = useCallback(
    async (rating: 'positive' | 'negative' | 'neutral', comment?: string) => {
      if (!onSubmitMessageFeedback || isUser || isPending || isStreaming) return;
      setFeedbackSubmitting(true);
      try {
        await onSubmitMessageFeedback(message.id, rating, comment);
        if (rating !== 'negative') {
    ...
    [isPending, isStreaming, isUser, message.id, onSubmitMessageFeedback]
  );
  ...
  return (
    ...
    >
      ...
      >
        ...
        {!isUser && hasIntelligence ? (
          <section className={localStyles.intelligencePanel} aria-label="Antwort-Qualität und Denkpfad">
            {message.confidence ? (
              <div className={localStyles.confidenceRow}>
                <span className={localStyles.confidenceBadge}>
                  Konfidenz: {(message.confidence.score * 100).toFixed(0)}% ({message.confidence.level})
                </span>
                {message.confidence.warnings.length > 0 ? (
                  <ul className={localStyles.confidenceWarnings}>
                    {message.confidence.warnings.slice(0, 3).map((warning, index) => (
                      <li key={`${message.id}-warn-${index}`}>{warning}</li>
                    ))}
                  </ul>
                ) : null}
              </div>
            ...
            {message.reasoningChain?.steps?.length ? (
              <details className={localStyles.reasoningDetails}>
                <summary className={localStyles.reasoningSummary}>
                  Denkpfad anzeigen ({message.reasoningChain.steps.length} Schritte)
                </summary>
                <ol className={localStyles.reasoningList}>
                  {message.reasoningChain.steps.map((step, index) => (
                    <li key={`${message.id}-step-${step.id ?? index}`}>
                      <strong>{step.label}</strong>
                      {step.detail ? <span className={localStyles.reasoningMeta}> — {step.detail}</span> : null}
                    </li>
                  ))}
                </ol>
              </details>
          ...
          <div className={localStyles.bubbleActionRow}>
            ...
            {onSubmitMessageFeedback ? (
              <>
                ...
                <button
                  type="button"
                  onClick={() => setShowNegativeFeedback(current => !current)}
                  className={localStyles.bubbleActionButton}
                  title="Antwort unzureichend"
                  aria-label="Antwort als unzureichend bewerten"
                  aria-expanded={showNegativeFeedback}
        ...
        {!isUser && onSubmitMessageFeedback && showNegativeFeedback ? (
          <div className={localStyles.feedbackPanel}>
            <label className={localStyles.feedbackLabel} htmlFor={`feedback-${message.id}`}>
              Was war nicht gut? (optional)
            </label>
            <textarea
              id={`feedback-${message.id}`}
              className={localStyles.feedbackTextarea}
              value={feedbackComment}
              onChange={event => setFeedbackComment(event.target.value)}
              rows={3}
              placeholder="z. B. unpräzise, wichtige Quelle fehlt, rechtlich unsicher"
            />
            <div className={localStyles.feedbackActions}>
              <button
                type="button"
                className={localStyles.feedbackSubmitButton}
                disabled={feedbackSubmitting}
                onClick={() => {
                  submitFeedback('negative', feedbackComment.trim() || undefined).catch(() => {});
                }}
              >
                Senden
              </button>
              <button
                type="button"
                className={localStyles.feedbackCancelButton}
                onClick={() => {
                  setShowNegativeFeedback(false);
                  setFeedbackComment('');
                }}
      ...
      {!isUser && message.artifacts && message.artifacts.length > 0 && (
        <ArtifactCards
          ...
          onSaveToAkte={
            onSaveArtifactToAkte
              ? artifact => {
                  Promise.resolve(onSaveArtifactToAkte(message.id, artifact)).catch(() => {});
                }
              : undefined
          }
        />
