...
export const LegalWorkflowSection = (props: Props) => {
  ...
  const deriveRelatedMatch = useCallback(
    (doc: LegalDocumentRecord): { matchedRef: string } | null => {
    ...
    [props.caseMatterAuthorityReferences]
  );
  ...
  const newVersionDocumentIds = useMemo(() => {
    ...
    for (const list of groups.values()) {
      ...
      for (let i = 1; i < sorted.length; i++) {
        const versionDocId = sorted[i]?.id;
        if (versionDocId) {
          out.add(versionDocId);
        }
      }
    }
    return out;
  }, [props.caseDocuments, deriveRelatedMatch]);
                ...
                <li className={styles.documentItem} key={document.id}>
                  ...
                  {document.contentFingerprint ? (
                    ...
                    >
                      ...
                      {document.ragIndexed ? (
                        <span title="Vektordatenbank: Chunks erfolgreich im pgvector Backend indiziert">
                          RAG ✓
                        </span>
                      ) : document.status === 'indexed' ? (
                        <span title="Vektordatenbank: Lokal indexiert (Backend-Sync ausstehend oder nicht verfügbar)">
                          RAG lokal
                        </span>
                      ) : null}
                    </div>
