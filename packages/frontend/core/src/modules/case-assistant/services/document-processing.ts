...
function extractStructuredText(
  ...
  if (ext === 'csv' || ext === 'tsv') {
    ...
    return {
      text: normalized,
      engine: ext === 'tsv' ? 'tsv-normalizer' : 'csv-normalizer',
    };
  }
 ...
 */
async function localOcrPdfAsync(base64: string): Promise<{
  text: string;
  pageCount: number;
  confidence: number;
  engine: string;
...
function extractWordTableText(tableXml: string): string {
  ...
  for (const rowMatch of tableXml.matchAll(/<w:tr[ >][\s\S]*?<\/w:tr>/g)) {
    ...
    for (const cellMatch of rowMatch[0].matchAll(
      ...
      const parts: string[] = [];
      for (const run of cellMatch[0].matchAll(
        /<w:t(?:\s[^>]*)?>([^<]*)<\/w:t>/g
      )) {
        parts.push(run[1]);
      }
...
function parseZipEntries(bytes: Uint8Array): Map<
  string,
  {
    offset: number;
 ...
 */
async function extractOdtFromBase64Async(
  base64: string
): Promise<{ text: string; pageCount: number | undefined }> {
  try {
    const decodePortion =
      base64.length > OFFICE_DECODE_LIMIT
        ? base64.slice(0, OFFICE_DECODE_LIMIT)
        : base64;
    ...
    return {
      ...
      pageCount:
        paraCount > 0 ? Math.max(1, Math.ceil(paraCount / 25)) : undefined,
    };
  } catch {
 ...
 */
function extractTextFromLegacyDoc(base64: string): {
  text: string;
  pageCount: number | undefined;
} {
  try {
    ...
    const textSegments = segments.filter(s => {
      ...
      const alphaRatio =
        (s.match(/[a-zA-ZäöüÄÖÜß]/g) ?? []).length / Math.max(1, s.length);
 ...
 */
function extractRtfFromBase64(base64: string): {
  text: string;
  pageCount: number | undefined;
} {
  try {
    const decoded = decodeBase64Text(base64);
    if (!decoded || decoded.trim().length < 5)
      return { text: '', pageCount: undefined };
...
async function extractDocxFromBase64Async(
  base64: string
): Promise<{ text: string; pageCount: number | undefined }> {
  try {
...
function extractWorksheetText(xml: string, sharedStrings: string[]): string {
  ...
  while ((inlineMatch = inlineRegex.exec(xml)) !== null) {
    const parts = [
      ...inlineMatch[0].matchAll(/<t(?:\s[^>]*)?>([\s\S]*?)<\/t>/g),
    ].map(v => v[1].replace(/\s+/g, ' ').trim());
    const text = parts.join(' ').trim();
    if (text) {
      cells.push(text);
    }
  ...
  const sharedRefRegex =
    /<c[^>]*\bt="s"[^>]*>[\s\S]*?<v>(\d+)<\/v>[\s\S]*?<\/c>/g;
...
async function extractXlsxFromBase64Async(
  base64: string
): Promise<{ text: string; pageCount: number | undefined }> {
  try {
    ...
    const sharedStrings = sharedStringsEntry
      ? extractSharedStringsFromXlsxXml(
          (await inflateZipEntryAsync(bytes, sharedStringsEntry)) ?? ''
        )
      : [];
    ...
    return {
      ...
      pageCount:
        worksheetEntries.length > 0 ? worksheetEntries.length : undefined,
    };
  } catch {
    return { text: '', pageCount: undefined };
  }
...
async function extractPptxFromBase64Async(
  base64: string
): Promise<{ text: string; pageCount: number | undefined }> {
  try {
...
function stripDangerousControlChars(input: string): string {
  let out = '';
  for (let i = 0; i < input.length; i++) {
    ...
    out += isAsciiControl && !isPreservedWhitespace ? ' ' : input[i];
  }
  return out;
}
...
export function normalizeText(input: string): string {
  return (
    stripDangerousControlChars(input.normalize('NFC'))
...
function detectLanguage(text: string): string {
  if (
    /\b(der|die|das|und|ist|nicht|wurde|frist|anspruch|gericht|urteil)\b/i.test(
      text
    )
  )
    return 'de';
  if (/\b(the|and|is|not|claim|liability|court|judgment)\b/i.test(text))
    return 'en';
  if (
    /\b(le|la|les|des|est|pas|tribunal|jugement|demande|contrat|partie)\b/i.test(
      text
    )
  )
    return 'fr';
  if (
    /\b(il|la|del|per|non|con|tribunale|sentenza|contratto|parte)\b/i.test(text)
  )
    return 'it';
...
function parsePipeTable(
  ...
  for (const line of lines) {
    ...
    const cells = line
      ...
      .filter(
        (_, idx, arr) =>
          idx > 0 && idx < arr.length - (line.endsWith('|') ? 1 : 0)
      );
...
function parseTabTable(
  ...
  const modeCol = colCounts.sort((a, b) => a - b)[
    Math.floor(colCounts.length / 2)
  ];
...
function detectHeadings(text: string): DetectedHeading[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (
      ...
      if (
        nextLine === '' ||
        (nextLine.length > 0 && nextLine !== nextLine.toUpperCase())
      ) {
 ...
 */
export function isLikelyBinaryGarbage(text: string): boolean {
  if (text.length < 10) return false;

  // ── Hard pattern matches for known binary format magic bytes ─────────────
  // JPEG: SOI marker ÿØ followed immediately by any APP marker ÿÀ–ÿÿ
  if (/ÿ[\u00C0-\u00FF]{0,3}ÿ/.test(text)) return true;
  // JPEG JFIF / Exif header identifiers
  if (text.includes('JFIF') || (text.includes('Exif') && text.includes('ÿ')))
    return true;
  // PNG magic bytes \x89PNG decoded as Latin-1 → ‰PNG
  if (text.includes('\u0089PNG') || text.includes('PNG\r\n\x1A')) return true;
  // GIF header GIF87a / GIF89a
  if (/^GIF8[79]a/.test(text)) return true;
  // ZIP / DOCX / XLSX PK header
  if (text.startsWith('PK\u0003\u0004') || text.startsWith('PK\u0005\u0006'))
    return true;
  // Binary cache placeholder from previous pipeline steps
  if (text.includes('__binary_cache__')) return true;
  // PDF binary stream: %PDF header + dense high-byte content
  if (/^%PDF-\d/.test(text) && /[\u0080-\u00FF]{20,}/.test(text.slice(0, 500)))
    return true;

  // ── Statistical analysis ──────────────────────────────────────────────────
  // High density of Latin Extended chars (À–ÿ, U+00C0–U+00FF).
  // Normal German prose: ~5-8 %. JPEG binary decoded as Latin-1: 30-60 %.
  const nonAsciiHighCount = (text.match(/[\u00C0-\u00FF]/g) ?? []).length;
  if (nonAsciiHighCount / text.length > 0.2) return true;

  // High density of C1 control / high-byte range (U+0080–U+00BF)
  const c1Count = (text.match(/[\u0080-\u00BF]/g) ?? []).length;
  if (c1Count / text.length > 0.1) return true;
  ...
  if (sample.length >= 20) {
    for (let n = 2; n <= 4; n++) {
      ...
      if (hits / windows > 0.4) return true;
    }
  }
...
function splitIntoChunks(text: string): string[] {
  ...
  for (const para of paragraphs) {
    ...
    if (
      current.length + trimmed.length + 1 > CHUNK_MAX_LENGTH &&
      current.length > 0
    ) {
  ...
  if (chunks.length === 1 && chunks[0].length > CHUNK_MAX_LENGTH) {
    ...
    for (const s of sentences) {
      if (
        block.length + s.length + 1 > CHUNK_TARGET_LENGTH &&
        block.length > 0
      ) {
...
function isLegalHeadingLine(line: string, nextLine: string): boolean {
  ...
  if (/^[IVXLC]{1,6}\.?\s+\S/.test(t) && t.length < 100 && !/[,;]$/.test(t))
    return true;

  // Letter headings: "A.", "B. Begründung"
  if (/^[A-Z]\.?\s+\S/.test(t) && t.length < 80 && !/[,;]$/.test(t))
    return true;
...
function splitIntoStructureAwareChunks(text: string): string[] {
  ...
  for (const section of sections) {
    ...
    const headingPrefix =
      sectionFirstLine.length < 120 ? sectionFirstLine + '\n' : '';
    ...
    for (let ci = 0; ci < subChunks.length; ci++) {
      if (ci === 0) {
        ...
        finalChunks.push(
          headingPrefix ? (headingPrefix + subChunks[ci]).trim() : subChunks[ci]
        );
      }
    }
  }
...
const CATEGORY_PATTERNS: Array<[SemanticChunkCategory, RegExp]> = [
  [
    'anklageschrift',
    /\b(anklageschrift|anklage erhoben|angeklagt wegen|anklagevorwurf|tatvorwurf)\b/i,
  ],
  [
    'strafanzeige',
    /\b(strafanzeige|anzeige erstattet|erstatte.*anzeige|strafantrag|strafanzeige gegen)\b/i,
  ],
  [
    'klageschrift',
    /\b(klageschrift|klage.*eingereicht|klagepartei|klageverfahren|klagebegründung)\b/i,
  ],
  [
    'berufung',
    /\b(berufung.*eingelegt|berufungsschrift|berufungsbegründung|berufungsverfahren|revision.*eingelegt)\b/i,
  ],
  [
    'antrag',
    /\b(beantrag|klageantrag|es wird beantragt|antrag auf|hiermit.*begehrt)\b/i,
  ],
  [
    'frist',
    /\b(frist|bis spätestens|innerhalb von|terminverlust|verjährung|ablauf)\b/i,
  ],
  [
    'sachverhalt',
    /\b(sachverhalt|tatbestand|zum sachverhalt|der kläger.*vorgetragen|der beklagte.*eingewandt)\b/i,
  ],
  [
    'rechtsausfuehrung',
    /\b(rechtsausführung|rechtlich|anspruchsgrundlage|§\s*\d|art\.\s*\d|haftung nach|gemäß)\b/i,
  ],
  ['begruendung', /\b(begründung|gründe|erwägung|aus.*gründen|im ergebnis)\b/i],
  [
    'urteil',
    /\b(urteil|beschluss|erkenntnis|im namen|recht erkannt|für recht erkannt)\b/i,
  ],
  [
    'bescheid',
    /\b(bescheid|verfügung|anordnung|spruch:|ergeht.*bescheid|steuerbescheid|abgabenbescheid|bußgeldbescheid)\b/i,
  ],
  [
    'protokoll',
    /\b(protokoll|verhandlungsprotokoll|sitzungsprotokoll|niederschrift über|verhandlung vom)\b/i,
  ],
  [
    'vollmacht',
    /\b(vollmacht|bevollmächtigt|hiermit bevollmächtige|vertretungsvollmacht|generalvollmacht|prozessvollmacht)\b/i,
  ],
  [
    'rechnung',
    /\b(rechnung|rechnungsnummer|nettobetrag|bruttobetrag|zahlbar bis|honorarnote|kostennote)\b/i,
  ],
  [
    'mahnung',
    /\b(mahnung|zahlungserinnerung|letzte mahnung|mahnbescheid|zahlungsaufforderung|inkasso|vollstreckungsbescheid|vollstreckungstitel|pfändung)\b/i,
  ],
  [
    'vertrag',
    /\b(vertrag|vereinbarung|die parteien.*vereinbaren|vertragsgegenstand|laufzeit|aufhebungsvertrag|abwicklungsvertrag|settlement\s*agreement)\b/i,
  ],
  [
    'korrespondenz',
    /\b(sehr geehrte|mit freundlichen|bezugnehmend|in bezug auf|ihr schreiben)\b/i,
  ],
  [
    'beweis',
    /\b(beweis|zeuge|sachverständig|gutachten|anlage|urkunde|urkundenbeweis)\b/i,
  ],
  ...
  [
    'klageschrift',
    /\b(kündigung.*ausgesprochen|kündigung.*erklärt|fristlose kündigung|ordentliche kündigung|kündigungsschreiben|das arbeitsverhältnis.*gekündigt)\b/i,
  ],
  [
    'begruendung',
    /\b(einspruch.*eingelegt|widerspruch eingelegt|einspruch gegen|beschwerde.*eingelegt|rechtsmittel.*eingelegt)\b/i,
  ],
  [
    'vertrag',
    /\b(freistellungsvereinbarung|freigestellt.*ab|ab.*datum.*freigestellt|unwiderruflich freigestellt)\b/i,
  ],
  [
    'bescheid',
    /\b(akteneinsicht|akteneinsichtnahme|einsicht in die akte|einsicht.*gewährt|zugang.*verweigert)\b/i,
  ],
  [
    'sachverhalt',
    /\b(vergleich.*geschlossen|vergleichsweise.*einigung|im wege des vergleichs|streit.*beigelegt|einigung erzielt)\b/i,
  ],
];
...
const LEGAL_REF_PATTERN =
  /(?:§§?\s*\d+[a-z]?(?:\s*(?:Abs|Abs\.|Absatz)\s*\d+)?(?:\s*(?:S|Satz|Z|Ziff|Ziffer|lit)\s*[.\d]+)?)\s*(?:BGB|StGB|ZPO|StPO|HGB|GmbHG|AktG|InsO|ABGB|StGB-AT|AVG|MRG|KSchG|WEG|UGB|EMRK|GG|B-VG|VfGG|VwGVG|VwGG|AußStrG|EO|IO|UrhG|MarkenG|PatG|BDSG|AGG|GewO|BetrVG|TVG|SGB|AO|UStG|EStG|GKG|RVG|ZGB|OR|SchKG|Code\s*civil|Code\s*pénal|KC|KK|KPC)\b/gi;
const AMOUNT_PATTERN =
  /(?:EUR|€|CHF|PLN)\s*[\d.,]+(?:\s*(?:Mio|Tsd|Mrd)\.?)?|\d[\d.,]*\s*(?:EUR|Euro|€|CHF|PLN)/gi;
const CASE_NUMBER_PATTERN =
  /\b\d{1,2}\s+(?:BvR|BvL|BvF|Ob|Os|Ra|Bka|Vfgh|VfSlg|VwSlg|C-)\s*\d+\/\d{2,4}[a-z]?\b/gi;
...
function extractEntities(text: string): ChunkExtractedEntities {
  const persons = [
    ...new Set(
      (text.match(PERSON_PATTERN) ?? []).map(s => s.replace(/\s+/g, ' ').trim())
    ),
  ];
  const organizations = [
    ...new Set(
      (text.match(ORGANIZATION_PATTERN) ?? []).map(s =>
        s.replace(/\s+/g, ' ').trim()
      )
    ),
  ];
  ...
  const legalRefs = [
    ...new Set((text.match(LEGAL_REF_PATTERN) ?? []).map(s => s.trim())),
  ];
  const amounts = [
    ...new Set((text.match(AMOUNT_PATTERN) ?? []).map(s => s.trim())),
  ];
  const caseNumbers = [
    ...new Set((text.match(CASE_NUMBER_PATTERN) ?? []).map(s => s.trim())),
  ];
  const addresses = [
    ...new Set(
      (text.match(ADDRESS_PATTERN) ?? []).map(s =>
        s.replace(/\s+/g, ' ').trim()
      )
    ),
  ];
  const ibans = [
    ...new Set(
      (text.match(IBAN_PATTERN) ?? []).map(s => s.replace(/\s+/g, '').trim())
    ),
  ];
  return {
    persons,
 ...
 */
function computeChunkQualityScore(
  text: string,
...
function extractKeywords(text: string): string[] {
  ...
  const LEGAL_BOOST_PATTERN =
    /\b(§§?\s*\d+[a-z]?|art\.\s*\d+|abs\.\s*\d+|haftung|schadensersatz|anspruch|kündigung|verjährungs|vollstreckung|pfändung|insolvenz|strafbar|delikt|bereicherung|verschulden|fahrlässig|vorsätzlich|rechtswidrig|schuld|unerlaubt)\b/gi;
...
function assessQuality(
  ...
  // Hard-fail: >50% non-word chars = clearly binary/corrupted content, not text.
  // Do NOT store such content in the document record.
  if (garbledRatio > 0.5) {
    problems.push({
      type: 'binary_content_rejected',
      description: `Binäre oder nicht lesbare Daten erkannt (${Math.round(garbledRatio * 100)}% nicht-textliche Zeichen) – Inhalt wurde abgelehnt.`,
      severity: 'error',
    });
    return { score: 0, problems };
  }
  ...
  if (wasOcr && text.length < 200 && kind === 'scan-pdf') {
    ...
    problems.push({
      ...
      description:
        'Sehr wenig Text aus Scan extrahiert — möglicherweise schlechte Bildqualität.',
      severity: 'warning',
    });
  }
  ...
  if (
    ...
    if (
      lastChar &&
      buildUnicodeRegex(String.raw`\p{L}`, 'u', /[A-Za-zÄÖÜäöüß]/).test(
        lastChar
      )
    ) {
      score -= 5;
...
function buildChecklist(
  ...
  items.push({
    ...
    detail:
      text.length > 10
        ? `${text.length} Zeichen extrahiert`
  ...
  items.push({
    ...
    status:
      quality.score >= 80 ? 'ok' : quality.score >= 50 ? 'warning' : 'error',
...
function extractPlainContent(
  ...
  if (
    kind === 'email' ||
    lowerTitle.endsWith('.eml') ||
    lowerTitle.endsWith('.msg')
  ) {
    return {
      extractedText: extractEmailBody(rawContent),
      extractionEngine: 'email-parser',
    };
  }
  if (
    ...
    return {
      extractedText: stripHtml(rawContent),
      extractionEngine: 'html-stripper',
    };
  }
  if (lowerTitle.endsWith('.rtf') || /^\{\\rtf/i.test(rawContent.trim())) {
    return {
      extractedText: stripRtf(rawContent),
      extractionEngine: 'rtf-stripper',
    };
  }
...
function buildPipelineResult(params: {
  ...
  const { input, wasOcr, extractedPageCount, startTime } = params;

  // ── Binary Garbage Guard ────────────────────────────────────────────────────
  // Reject raw JPEG/PNG/ZIP/binary data decoded as Latin-1 text before it
  // contaminates normalizedText, semantic chunks, and the stored document record.
  // This must run BEFORE normalizeText(), which cannot recover binary content.
  let extractedText = params.extractedText;
  let extractionEngine = params.extractionEngine;
  if (isLikelyBinaryGarbage(extractedText)) {
    extractedText = '';
    extractionEngine = `${extractionEngine}:binary-rejected`;
  }
  ...
  const chunkTexts =
    normalizedText.length > 0
      ? splitIntoStructureAwareChunks(normalizedText)
      : [];
  ...
  const allEntities: ChunkExtractedEntities = {
    ...
    organizations: [
      ...new Set(chunks.flatMap(c => c.extractedEntities.organizations)),
    ],
    ...
    caseNumbers: [
      ...new Set(chunks.flatMap(c => c.extractedEntities.caseNumbers)),
    ],
  ...
  const structure =
    normalizedText.length > 50
      ? analyzeTextStructure(normalizedText)
      : undefined;
  ...
  if (structure) {
    if (structure.headings.length > 0)
      quality.score = Math.min(100, quality.score + 3);
    if (structure.tables.length > 0)
      quality.score = Math.min(100, quality.score + 2);
    if (structure.hasColumnLayout) {
      quality.score = Math.max(0, quality.score - 5);
      quality.problems.push({
        type: 'column_layout_detected',
        description:
          'Spaltenlayout erkannt — Lesereihenfolge möglicherweise fehlerhaft.',
        severity: 'warning',
      });
    }
  }
  ...
  const totalEntities =
    ...
    allEntities.dates.length +
    allEntities.legalRefs.length +
    allEntities.amounts.length +
    allEntities.caseNumbers.length +
    allEntities.addresses.length +
    allEntities.ibans.length;
  ...
  const estimatedOverlapChars =
    chunks.length > 1 ? (chunks.length - 1) * CHUNK_OVERLAP : 0;
  const effectiveChunkChars = Math.max(
    0,
    totalChunkChars - estimatedOverlapChars
  );
...
export async function processDocumentPipeline(
  ...
  if (isBase64DataUrl(input.rawContent)) {
    ...
    if (!base64) {
      extractedText = '';
      extractionEngine = 'base64-invalid';
      wasOcr = true;
    } else if (isRtf) {
      ...
      extractedText = r.text;
      extractionEngine =
        r.text.length > 20 ? 'doc-legacy-parser' : 'doc-legacy-empty';
      extractedPageCount = r.pageCount ?? extractedPageCount;
    } else if (isDocx) {
      const r = await extractDocxFromBase64Async(base64);
      extractedText = r.text;
      extractionEngine = r.text.length > 50 ? 'docx-parser' : 'docx-empty';
      extractedPageCount = r.pageCount ?? extractedPageCount;
    } else if (isXlsx) {
      const r = await extractXlsxFromBase64Async(base64);
      extractedText = r.text;
      extractionEngine =
        r.text.trim().length > 0 ? 'xlsx-parser' : 'xlsx-empty';
      extractedPageCount = r.pageCount ?? extractedPageCount;
    } else if (isPptx) {
      const r = await extractPptxFromBase64Async(base64);
      extractedText = r.text;
      extractionEngine =
        r.text.trim().length > 0 ? 'pptx-parser' : 'pptx-empty';
      extractedPageCount = r.pageCount ?? extractedPageCount;
      ...
      if (deepResult.encrypted) {
        ...
        if (workerExtraction && workerExtraction.text.trim().length > 20) {
          ...
          extractedPageCount =
            workerExtraction.pageCount ??
            deepResult.pageCount ??
            extractedPageCount;
        } else {
...
export class DocumentProcessingService extends Service {
  ...
  async processDocumentAsync(input: {
    ...
    const isDocx =
      isActualDocx ||
      (input.kind === 'docx' && !isLegacyDoc && !isOdt && !isRtf);
    const isXlsx =
      input.kind === 'xlsx' ||
      ext === 'xlsx' ||
      ext === 'xls' ||
    ...
    if (isBase64DataUrl(input.rawContent)) {
      ...
      if (!base64) {
        ...
        extractionEngine =
          result.text.length > 50 ? 'docx-deep-parser' : 'docx-empty';
        wasOcr = false;
      } else if (isXlsx) {
        ...
        extractedPageCount = extractedPageCount ?? result.pageCount;
        extractionEngine =
          result.text.trim().length > 0 ? 'xlsx-parser' : 'xlsx-empty';
      } else if (isPptx) {
        const result = await extractPptxFromBase64Async(base64);
        extractedText = result.text;
        extractedPageCount = extractedPageCount ?? result.pageCount;
        extractionEngine =
          result.text.trim().length > 0 ? 'pptx-parser' : 'pptx-empty';
      } else if (isStructured) {
        const decoded = decodeBase64Text(base64);
        const structured = extractStructuredText(decoded, ext);
        extractedText = structured.text;
        ...
        if (deepPdfExtraction.encrypted) {
          ...
          const workerExtraction =
            await extractTextFromPdfiumWorkerAsync(base64);
        ...
        if (decoded.trim().length > 20) {
          ...
          extractionEngine = 'binary-decoded-text';
        } else {
          extractedText = '';
      ...
      ({ extractedText, extractionEngine } = extractPlainContent(
        input.rawContent,
        input.kind,
        lowerTitle
      ));
    ...
    if (isBase64DataUrl(input.rawContent)) {
      ...
      if (!base64) {
        ...
        if (result.encrypted) {
          ...
          extractionEngine =
            result.text.length > 50 ? 'pdf-deep-parser' : 'pdf-no-text-layer';
