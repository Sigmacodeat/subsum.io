...
type GoldenPdfFixture = {
  ...
  shouldFindDates?: boolean;
  shouldFindPersons?: boolean;
  shouldFindLegalRefs?: boolean;
  allowFailedProcessing?: boolean;
};

const TEST_PDFS: GoldenPdfFixture[] = [
  {
  ...
  },
  {
    ...
    label: 'repo-fixture-lorem-ipsum',
    kind: 'pdf',
    baseDir: resolve(REPO_ROOT, 'tests/fixtures'),
    expectedMinChars: 20,
    expectedMinChunks: 1,
    shouldFindDates: false,
    allowFailedProcessing: false,
  },
  {
    file: 'sample.pdf',
    label: 'repo-fixture-sample-pdf',
    kind: 'pdf',
    baseDir: resolve(REPO_ROOT, 'packages/common/native/fixtures'),
    expectedMinChars: 20,
    expectedMinChunks: 1,
    shouldFindDates: false,
    allowFailedProcessing: false,
...
describe.skipIf(!SHOULD_RUN_REAL_PDF_E2E)(
  'E2E PDF Pipeline — Real PDFs from ~/Desktop/Akt neu',
  () => {
    let availablePdfs: typeof TEST_PDFS;

    function resolvePdfPath(pdf: GoldenPdfFixture) {
      return resolve(pdf.baseDir, pdf.file);
    }
    ...
    for (const pdf of TEST_PDFS) {
      const filePath = resolvePdfPath(pdf);
      ...
      test(`[${pdf.label}] extracts text from real PDF`, async () => {
        ...
        console.log(
          `  normalizedText preview: "${result.normalizedText.slice(0, 200)}..."`
        );

        expect(result.normalizedText.length).toBeGreaterThanOrEqual(
          pdf.expectedMinChars
        );
        ...
        if (result.normalizedText.length > 50 && !pdf.allowFailedProcessing) {
        ...
        console.log(
          `  entities: ${allPersons.length} persons, ${allDates.length} dates, ${allLegalRefs.length} legalRefs, ${allAmounts.length} amounts, ${allOrgs.length} orgs, ${allAddresses.length} addresses`
        );
    ...
    test('normalizeText preserves paragraph breaks in extracted text', async () => {
      ...
      const pdf =
        TEST_PDFS.find(candidate => existsSync(resolvePdfPath(candidate))) ??
        TEST_PDFS[0];
      const filePath = resolvePdfPath(pdf);
      if (!existsSync(filePath)) {
      ...
      console.log(
        `✅ normalizeText structure preserved (${text.split('\n').length} lines)`
      );
    }, 60_000);
    ...
    test('chunk quality scores are distributed (not all identical)', async () => {
      ...
      const pdf =
        TEST_PDFS.find(
          p =>
            p.label === 'medium-haftverhandlung' &&
            existsSync(resolvePdfPath(p))
        ) ??
        TEST_PDFS.find(candidate => existsSync(resolvePdfPath(candidate))) ??
        TEST_PDFS[0];
      const filePath = resolvePdfPath(pdf);
      ...
      if (result.chunks.length >= 3) {
        ...
        console.log(
          `  Chunk scores: ${scores.map(s => s.toFixed(2)).join(', ')}`
        );
        expect(uniqueScores.size).toBeGreaterThan(1);
      }

      console.log(
        `✅ Quality score distribution verified (${result.chunks.length} chunks)`
      );
    }, 60_000);
    ...
    test('full pipeline summary across all test PDFs', async () => {
      ...
      if (availablePdfs.length === 0) {
        console.warn(
          'Skipping full pipeline summary — no PDF fixtures available'
        );
        return;
      }
...
`);
      ...
      expect(
        totalPersons + totalDates + totalLegalRefs + totalAmounts
      ).toBeGreaterThan(0);
